...
const VDLayout& VirtualDesk::activeLayout(const RememberLayoutConf& conf, const CSharedPointer<CMonitor>& exclude) {
    if (!activeIsValid) {
        activeIsValid = true;
...
VDLayout& VirtualDesk::searchActiveLayout(const RememberLayoutConf& conf, const CSharedPointer<CMonitor>& exclude) {
...
CSharedPointer<CMonitor> VirtualDesk::deleteInvalidMonitor(const CSharedPointer<CMonitor>& monitor) {
    VDLayout layout_copy(layouts[m_activeLayout_idx]);
...
void VirtualDesk::checkAndAdaptLayout(VDLayout* layout, const CSharedPointer<CMonitor>& exclude) {
    auto enabledMons = currentlyEnabledMonitors(exclude);
    if (enabledMons.empty())
        return;
    for (const auto& [mon, wid] : VDLayout(*layout)) {
...
VDLayout VirtualDesk::generateCurrentMonitorLayout() {
    VDLayout layout;
    ...
    int  j                   = 0;
    for (WORKSPACEID i = vdeskFirstWorkspace; i < vdeskFirstWorkspace + monitors.size(); i++) {
        layout[monitors[j]] = i;
        j++;
    }
    return layout;
